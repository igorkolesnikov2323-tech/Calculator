...
buttons.forEach(btn => {    
    btn.addEventListener('click', function ddd(event) {
        ...
        if (!qual.includes(t) && !procent.includes(t) && !opposite.includes(t) && !backspace.includes(t)){
            input.value=a+=t 
        }
        if(qual.includes(t)){ // Button =
            input.value=(eval(a))
            a = eval(a)
